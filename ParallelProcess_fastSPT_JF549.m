%   ParallelProcess_fastSPT_JF549.m
...
if exist(output_path,'dir')
    ...
    disp('The given output folder exists. MATLAB workspaces will be saved to:');
    disp(output_path);
else
    mkdir(output_path);
    disp('The given output folder did not exist, but was just created. MATLAB workspaces will be saved to:');
    disp(output_path);
end

%%%%% DEFINE THE KEY PARAMETERS %%%%%
NumWorkers = 7; % input the maximum number of workers available on your computer: too many and you will run out of memory
...
ExposureTime = 13.447; % in milliseconds
